...
func TestCorp104OpenIDConnectExplicitFlow(t *testing.T) {
	// key type not supported: neither *rsa.PrivateKey nor *ecdsa.PrivateKey
	assert.Panics(t, func() {
		compose.Corp104ComposeAllEnabled(new(compose.Config), fositeStore, []byte("some-secret-thats-random-some-secret-thats-random-"), nil)
	})

	fs := []fosite.OAuth2Provider{
		compose.Corp104ComposeAllEnabled(new(compose.Config), fositeStore, []byte("some-secret-thats-random-some-secret-thats-random-"), internal.MustECDSAKey()),
		compose.Corp104ComposeAllEnabled(new(compose.Config), fositeStore, []byte("some-secret-thats-random-some-secret-thats-random-"), internal.MustRSAKey()),
	}
